 ...
 *
 * $LastChangedDate: 2007-09-22 14:56:10 +0100 (Sat, 22 Sep 2007) $
 * $Rev: 3436 $
 *
 * Modifications made 2011-01-11 for EEMs needs. 
 */
...
jQuery.datePicker = function()
{
	...
	var navLinks = {p:'Prev', n:'Next', c:'[ X ]', b:'Choose date', t:'Todays date'};
	var dateFormat = 'dmy';
	var dateSeparator = "/";
	...
	var _getCalendarDiv = function(dIn)
	{
      ...
      jQuery("<h3></h3>").html(months[d.getMonth()] + '<br> ' + d.getFullYear())
//			jQuery("<div></div>").attr('class', 'link-close').append(nextLink)
		...
		jCalDiv.append(
			jQuery("<table width=100%></table>").attr('cellspacing',2).append("<thead></thead>")
...
//	).append(prevLinkDiv).append(nextLinkDiv);
		...
		if (jQuery.browser.msie) {
			...
			var iframe = [	'<iframe class="bgiframe" tabindex="-1" src="/_style/css/blank.html" ',
    ...
    getTodayDateStr: function(){
      ...
      var fDate = now.getFullYear()+'-'+(now.getMonth()+1)+'-'+now.getDate(); 
          fDate += ' '+now.getHours()+':'+now.getMinutes();      
    
      jQuery(input).val(fDate);
		},
		show: function(){
			if (_openCal) {
				_closeDatePicker();
      ...
      _openCal = jQuery(this).parent().find('>div.popup-calendar-wrapper');
		...
		select2Day: function(ele)
		{
      //var input = jQuery('input', jQuery(this).findClosestParent('input')[0])[0];
      alert(_retID);
      //alert(this.show._firstDate);
      //alert(input.id);      
			//var $theInput = jQuery('input', jQuery(ele).findClosestParent('input')[0]);
			//$theInput.val('test');
...
jQuery.fn.datePicker = function(a){
	this.each(function() {
      ...
      var todayDate = jQuery.datePicker.getTodayDateStr();
      var calBut;
      var calBut2;
			if(a && a.inputClick){
        calBut = jQuery(this).attr('title', chooseDate).addClass('date-picker');
				calBut2 = jQuery(this).attr('title', todayDate).addClass('date-picker2Day');
			}
			else {
				calBut = jQuery("<a></a>").attr({'href':'javascript:;','class':'date-picker', 'title':chooseDate})
				calBut2 = jQuery("<a></a>").attr({'href':'javascript:;','class':'date-picker2Day', 'title':todayDate})
      ...
      calBut2.bind('click', jQuery.datePicker.setDate);
